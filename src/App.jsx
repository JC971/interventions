...
const OfflineBanner = () => (
	<div style={styles.offlineBanner}>
		<span>
			📡 Vous êtes hors ligne - Les données sont sauvegardées localement
		</span>
	</div>
);
...
const InstallBanner = ({ onInstall, onDismiss }) => (
	<div style={styles.installBanner}>
		...
		<div style={styles.installBannerButtons}>
			<button onClick={onDismiss} style={styles.installBannerDismiss}>
				Plus tard
			</button>
			<button onClick={onInstall} style={styles.installBannerAccept}>
				Installer
			</button>
		</div>
	</div>
...
const Badge = ({ etat }) => (
	<span
		style={{
			...styles.badge,
			backgroundColor: ETAT_COLORS[etat]?.bg || "#6b7280",
		}}
	>
...
const InterventionCard = memo(
	({ intervention, currentUser, onEdit, onDelete, isMobile }) => {
		const canEdit =
			currentUser.role === "admin" ||
			intervention.intervenant === currentUser.name;
			...
			<div style={styles.card}>
				...
				<div style={styles.cardBody}>
					...
					<InfoRow
						icon="📅"
						label="Date"
						value={new Date(intervention.date).toLocaleDateString("fr-FR")}
						isMobile={isMobile}
					/>
					<InfoRow
						icon="👤"
					...
					<InfoRow
						icon="🕐"
						label="Horaires"
						value={`${intervention.heureDebut} - ${intervention.heureFin}`}
						isMobile={isMobile}
					/>
		...
		<div style={styles.overlay}>
			...
			>
				...
				<form
					...
					style={{ ...styles.form, padding: isMobile ? "1rem" : "1.5rem" }}
				>
					...
					<button
						type="submit"
						style={{ ...styles.btnPrimary, ...styles.btnMobile, width: "100%" }}
					>
						Se connecter
					</button>
				</form>
				...
				>
					...
					<div style={styles.accountsList}>
						<div style={styles.accountItem}>
							...
							<code style={{ fontSize: isMobile ? "0.8rem" : "0.85rem" }}>
								admin / admin123
							</code>
						</div>
						<div style={styles.accountItem}>
				...
				>
					<div style={styles.formGrid}>
						<div style={styles.formGroup}>
							...
							/>
							{errors.bien && (
								<span style={styles.errorText}>{errors.bien}</span>
							)}
						</div>
						...
						<div style={styles.formGroup}>
							...
							/>
							{errors.adresse && (
								<span style={styles.errorText}>{errors.adresse}</span>
							)}
						</div>
							...
							<div style={styles.formGroup}>
								...
								<input
									style={{
										...styles.input,
										...styles.inputMobile,
										...(errors.heureDebut && styles.inputError),
									}}
									type="time"
								...
								/>
								{errors.heureDebut && (
									<span style={styles.errorText}>{errors.heureDebut}</span>
								)}
							</div>
							...
							<div style={styles.formGroup}>
								...
								<input
									style={{
										...styles.input,
										...styles.inputMobile,
										...(errors.heureFin && styles.inputError),
									}}
									type="time"
									value={form.heureFin}
								...
								/>
								{errors.heureFin && (
									<span style={styles.errorText}>{errors.heureFin}</span>
								)}
							</div>
						</div>
						...
						{form.heureDebut && form.heureFin && (
							<div style={styles.durationPreview}>
								...
								<span style={styles.durationPreviewLabel}>
									Durée de l'intervention :
								</span>
								<span style={styles.durationPreviewValue}>
									{previewDurationText}
								</span>
							</div>
						)}
							...
							<div style={styles.formGroup}>
								...
								<select
									style={{
										...styles.input,
										...styles.inputMobile,
										...(errors.intervenant && styles.inputError),
									}}
								...
								{errors.intervenant && (
									<span style={styles.errorText}>{errors.intervenant}</span>
								)}
							</div>
						)}
	...
	<header style={styles.headerMobile}>
		<div style={styles.headerMobileTop}>
			<div style={styles.headerLeft}>
				...
				<h1 style={{ ...styles.headerTitle, fontSize: "1.1rem" }}>
					Interventions
				</h1>
			</div>
...
const App = () => {
	...
	const handleDeleteIntervention = useCallback((id) => {
		if (
			window.confirm("Êtes-vous sûr de vouloir supprimer cette intervention ?")
		) {
	...
	const handleSync = async () => {
		if (!isOnline) {
			alert(
				"Vous êtes hors ligne. Connectez-vous à internet pour synchroniser.",
			);
			return;
		}
...
const styles = {
	...
	// Label styles pour les champs du formulaire
	label: {
		display: "block",
		marginBottom: "0.5rem",
		fontWeight: 600,
